...
int main(void)
{
    ...
    while (!isGameOver)
    {
        ...
        char command[2048];
        static void sendToGoogleSheet(const char* nickname_initial, const char* currentDateTime, const char* endDateTime, int level, int TicketBuyLevels[], int attempt, int success, int selling_count, int money); {
            sprintf_s(command, sizeof(command),
                "curl -d \"{\\\"아이디\\\":\\\"%s\\\",\\\"접속시간\\\":\\\"%d\\\",\\\"종료시간\\\":\\\"%d\\\",\\\"강화 단계\\\":\\\"%d\\\",\\\"단계별 복습권 구매갯수\\\":\\\"%d\\\",\\\"강화시도횟수\\\":\\\"%d\\\",\\\"성공 횟수\\\":\\\"%d\\\",\\\"판매 횟수\\\":\\\"%d\\\",\\\"현재 비용\\\":\\\"%d\\\"}\" https://script.google.com/macros/s/AKfycbzQ2hkBUVNDO_azp-G7XfxCVoyT4Qcrf5K4veqqL7C9-uDPz7_IjuwwFzzsA6lUclP4/exec",
                nickname_initial, currentDateTime, endDateTime, level, TicketBuyLevels[level], attempt, success, selling_count, money);
